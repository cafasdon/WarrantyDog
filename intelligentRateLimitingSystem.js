...
class IntelligentRateLimitingSystem {
    constructor(vendor, config = {}) {
        ...
        this.optimizationInterval = null; // Store interval reference for cleanup
    ...
    async recordSuccess(requestId, responseTime, plan, context) {
        ...
        // End burst if active
        if (plan.useBurst && this.burstManager.isInBurstMode) {
    ...
    stopOptimizationLoop() {
        if (this.optimizationInterval) {
            ...
            console.log(`[${this.vendor}] Optimization loop stopped`);
        }
    }
