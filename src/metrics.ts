...
interface MetricsSnapshot {
  ...
  rateLimiting: MetricsData['rateLimiting'];
  system: {
    memory: {
      rss: number;
      heapTotal: number;
...
class MetricsCollector {
   ...
   */
  recordRequest(method: string, endpoint: string, statusCode: number, responseTime?: number, ip?: string): void {
   ...
   */
  recordVendorApiCall(vendor: string, responseTime?: number, isError: boolean = false, isRateLimit: boolean = false): void {
    const vendorKey = vendor.toLowerCase() as keyof typeof this.metrics.vendorApis;
    const vendorMetrics = this.metrics.vendorApis[vendorKey];
    if (vendorMetrics) {
      vendorMetrics.calls++;
      if (responseTime) {
   ...
   */
  recordDatabaseOperation(queryTime?: number, isError: boolean = false): void {
   ...
   */
  recordRateLimitBlocked(ip: string): void {
   ...
   * Record error
   */
  recordError(statusCode: number, endpoint: string): void {
   ...
   */
  private calculateRate(count: number, uptimeMs: number): number {
